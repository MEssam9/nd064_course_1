...
app.config['DB_COUNT'] = app.config.get('DB_COUNT', 0)
...
@app.route('/metrics')
def metrics():
    # function to get the number of posts in sqlite3
    posts = get_db_connection().execute('SELECT * FROM posts').fetchall()
    # list count
    post_count = len(posts)
    response = app.response_class(
        response=json.dumps(
            {"post_count": post_count, "db_connection_count": app.config['DB_COUNT']}),
        status=200,
        mimetype='application/json'
    )
    return response
